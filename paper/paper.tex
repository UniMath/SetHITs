...
Using Lemmata \ref{lem:prealgadj} and \ref{lem:comm}, we lift lift both the quotient and the path setoid functors to obtain.
\[
...
In addition, given is a functor $R : \functor{\mathcal{D}}{\mathcal{C}}$ together with a natural transformation $m : \nattrans{\compf{R}{A_1}}{\compf{A_2}{L}}$ and an adjunction $\adj{L}{R}$ with unit $\eta$ and counit $\varepsilon$.
We also assume the following
\begin{itemize}
	\item For each $X : \mathcal{C}$, the following diagram commutes
	\[
	\xymatrix
	{	
		A_2 (L(R \> X)) \ar[dr]^{\epsilon \> X} \ar[d]_{\theta_1} \\
		R(L(A_2 \> X)) \ar[r]_{\epsilon(A_2 \> X)} & A_2 \> X
	}
	\]
	where the transformation $\theta_1$ is defined using whiskering, associativity of composition, and $n$ and $m$.
	\item For each $X : \mathcal{D}$, the following diagram commutes
	\[
	\xymatrix
	{
		A_1 \> X \ar[dr]_{\eta \> (A_1 \> X)} \ar[r]^{A_1 \> (\eta \> X)} & A_1(R(L \> X)) \ar[d]^{\theta_2}\\
		& R(L(A_1 \> X))
	}
	\]
	where the transformation $\theta_2$ is defined using whiskering, associativity of composition, and $n$ and $m$.
\end{itemize}
With all this, for each $X : \mathcal{D}$ the map $\eta \> X$ is an algebra homomorphism and for each $Y : \mathcal{C}$ the map $\epsilon \> X$ is an algebra homomorphism.
...
Now we obtain the desired adjunction.
